...
import { EdgeType, NodeType } from '@/hooks/useGraphsuiteGraph'
...
const GraphControl = ({ disableHoverEffect }: { disableHoverEffect?: boolean }) => {
  ...
  useEffect(() => {
    if (sigmaGraph && sigma) {
      ...
      try {
        // Check if sigma instance is still valid
        const container = sigma.getContainer()
        if (!container || container.offsetWidth === 0) {
          console.warn('Sigma container is invalid, skipping graph binding')
          return
        }
