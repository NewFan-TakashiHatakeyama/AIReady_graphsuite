...
const NodeOption = ({ id }: { id: string }) => {
  ...
  try {
    // Check if graph exists and has the node
    if (!graph?.hasNode(id)) {
      return null
    }
    
    // Check if sigma instance exists and is properly initialized
    if (!sigmaInstance) {
      return null
    }
    ...
    console.warn('Error rendering NodeOption for id:', id, error)
    return null
  }
}
...
const EdgeOption = ({ id }: { id: string }) => {
  ...
  try {
    // Check if graph exists and has the edge
    if (!graph?.hasEdge(id)) {
      return null
    }
    ...
    return <EdgeById id={id} />
  } catch (error) {
    console.warn('Error rendering EdgeOption for id:', id, error)
    return null
  }
}
...
export const GraphSearchInput = ({
  ...
  return (
    <AsyncSearch
      className="liquid-glass-surface liquid-glass-clear w-24 rounded-xl border border-white/30 opacity-75 transition-all hover:w-fit hover:opacity-100"
      fetcher={loadOptions}
