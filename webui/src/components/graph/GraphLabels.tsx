...
import { getGraphLabels } from '@/api/graphsuite'

const GraphLabels = ({ source = 'ontology' }: { source?: 'default' | 'ontology' }) => {
  ...
  useEffect(() => {
    ...
    if (labelsFetchAttempted) {
      ...
      } else if (source !== 'ontology' && label && allDatabaseLabels.length <= 1 && label && label !== '*') {
  ...
  }, [allDatabaseLabels, label, labelsFetchAttempted, source]);
