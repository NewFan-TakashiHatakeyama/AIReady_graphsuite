...
pytestmark = [
    pytest.mark.aws,
    pytest.mark.skip(reason="Phase 6.5 detectSensitivity pipeline is not active after hard-cut"),
]
...
class TestE2E4Phase65:
    ...
    def test_e2e_4_02_vectors_object_exists(
        ...
        assert key.startswith(f"{tenant_id}/vectors/{item_id}")
    ...
    def test_e2e_4_04_detect_sensitivity_env_has_ontology_extensions(self, lambda_client):
        """detectSensitivity にオントロジー連携用の環境変数が設定されていること。"""
        cfg = lambda_client.get_function(FunctionName=DETECT_SENSITIVITY_FN)["Configuration"]
        env = cfg["Environment"]["Variables"]
        assert env.get("DOCUMENT_ANALYSIS_TABLE_NAME")
        assert env.get("VECTORS_BUCKET")
        assert env.get("ENTITY_RESOLUTION_QUEUE_URL")
