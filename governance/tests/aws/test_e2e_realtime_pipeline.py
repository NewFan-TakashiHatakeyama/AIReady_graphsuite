...
class TestE2E1RealtimePipeline:
    ...
    @pytest.mark.slow
    @pytest.mark.skip(reason="detectSensitivity-dependent checks are retired in hard-cut mode")
    ...
    def test_e2e_1_04_permission_change_rescore(
        ...
        connect_table.update_item(
            Key={"drive_id": metadata["drive_id"], "item_id": item_id},
    ...
    def test_e2e_1_05_deletion_closes(
        ...
        connect_table.delete_item(Key={"drive_id": metadata["drive_id"], "item_id": item_id})
    ...
    def test_e2e_1_06_dlq_empty(self, sqs_client, analyze_dlq_url):
        """全テスト完了後に analyzeExposure DLQ にメッセージが残っていないことを確認。"""
        ...
        assert visible == 0 and not_visible == 0, (
            f"{ANALYZE_DLQ_NAME}: visible={visible}, not_visible={not_visible}"
        )
